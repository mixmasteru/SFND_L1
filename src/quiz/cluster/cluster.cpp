...
void clusterIt(int index, const std::vector<std::vector<float>> points, std::vector<int> &cluster,
    ...
    for (int id: nearest) {
        if (!processed[id]) {
            clusterIt(id, points, cluster, processed, tree, distanceTol);
        }
    }
...
euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol) {
    ...
    while (i < points.size()) {
        ...
        clusterIt(i, points, cluster, processed, tree, distanceTol);
